...
package uk.gov.hmrc.test.ui.specpage.deregistration
...
object DeRegistrationPage extends BasePage {
  ...
  def getPaymentAndReturnLinks(linkName: String = "Returns"): this.type = {
    linkName match {
      case "Payments" =>
        val paymentElements = Driver.instance.findElements(By.linkText("Make a payment"))
        Assert.assertTrue(paymentElements.size() > 0)

        val elements = Driver.instance.findElements(By.linkText("Request a refund"))
        Assert.assertTrue(elements.size() > 0)
      case _          =>
        val amendElements = Driver.instance.findElements(By.linkText("Amend return"))
